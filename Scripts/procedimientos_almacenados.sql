...
-- Usamos FUNCTION para consultas porque es más compatible con JDBC
...
-- =====================================================
-- PROCEDIMIENTOS ALMACENADOS PARA GESTIÓN DE EMPRESAS
-- =====================================================
...
-- =====================================================
-- PROCEDIMIENTOS ALMACENADOS PARA GESTIÓN DE ROLES
-- =====================================================
...
-- USUARIOS:
-- Listar usuarios activos (FUNCTION - retorna tabla):
-- SELECT * FROM sp_listar_usuarios_activos();

-- Obtener usuario por ID (FUNCTION - retorna tabla):
-- SELECT * FROM sp_obtener_usuario_por_id(1);

-- Guardar nuevo usuario (PROCEDURE - usa parámetros OUT):
-- CALL sp_guardar_usuario(0, 'juan.perez', 'password123', 'Juan', 'Pérez', 'García', 1, NULL, 1, '12345678', '1990-01-01', 1, 1, '987654321', 'juan@empresa.com', NULL, NULL, NULL);

-- Eliminar usuario (PROCEDURE - usa parámetros OUT):
-- CALL sp_eliminar_usuario(1, NULL, NULL);

-- EMPRESAS:
-- Listar empresas activas (FUNCTION - retorna tabla):
-- SELECT * FROM sp_listar_empresas_activas();

-- Obtener empresa por ID (FUNCTION - retorna tabla):
-- SELECT * FROM sp_obtener_empresa_por_id(1);

-- Guardar nueva empresa (PROCEDURE - usa parámetros OUT):
-- CALL sp_guardar_empresa(0, 'Mi Empresa SAC', '20123456789', '01-1234567', 'Av. Principal 123', NULL, NULL);

-- Eliminar empresa (PROCEDURE - usa parámetros OUT):
-- CALL sp_eliminar_empresa(1, NULL, NULL);

-- ROLES:
-- Listar roles activos (FUNCTION - retorna tabla):
-- SELECT * FROM sp_listar_roles_activos();

-- Obtener rol por ID (FUNCTION - retorna tabla):
-- SELECT * FROM sp_obtener_rol_por_id(1);

-- Guardar nuevo rol (PROCEDURE - usa parámetros OUT):
-- CALL sp_guardar_rol(0, 'Super Administrador', NULL, NULL);

-- Eliminar rol (PROCEDURE - usa parámetros OUT):
-- CALL sp_eliminar_rol(1, NULL, NULL);

-- =====================================================
-- NOTAS IMPORTANTES
-- =====================================================
-- 
-- ¿Por qué FUNCTION para consultas y PROCEDURE para operaciones?
--
-- 1. FUNCTION con RETURNS TABLE:
--    - Retorna datos directamente como una tabla
--    - Se puede usar con SELECT * FROM funcion()
--    - Más eficiente para consultas
--    - Compatible con JDBC sin configuración especial
--    - Es el estándar de PostgreSQL para consultas
--
-- 2. PROCEDURE con parámetros OUT:
--    - Ideal para operaciones de escritura (INSERT/UPDATE/DELETE)
--    - Puede retornar múltiples valores con OUT
--    - Mejor control de transacciones
--    - Se llama con CALL procedimiento()
--
-- 3. Ventajas de este enfoque híbrido:
--    ✅ Fácil de modificar (CREATE OR REPLACE funciona siempre)
--    ✅ Mejor rendimiento
--    ✅ Más compatible con diferentes lenguajes (Java, C#, Python, etc.)
--    ✅ Código más limpio y mantenible
--    ✅ Sigue las mejores prácticas de PostgreSQL
