...
const consultaAsistencia = {
    ...
    init: function() {
        console.log('✅ Módulo Consulta Asistencia inicializado');
    ...
    consultar: async function() {
        ...
        try {
            const empresaId = localStorage.getItem('empresa_id') || window.EMPRESA_ID || 1;
            const url = `http://localhost:3000/api/asistencias/consulta-trabajador?` +
    ...
    renderizarTabla: function(datos) {
        const tbody = $('#tablaConsultaAsistencia tbody');
        tbody.empty();
...
if (document.querySelector('main')) {
    observerConsultaAsistencia.observe(document.querySelector('main'), {
        childList: true,
        subtree: true
    });
}
